...
def _sma_at_or_before(sma_series: pd.Series, ts: pd.Timestamp) -> float | None:
    ...
    val = cast(float | None, sma_series.asof(ts))
...
def run_config(
    ...
    if sma_period > 0:
        ...
        sma_15m_series = cast(
            pd.Series,
            data_15m["close"].rolling(window=sma_period, min_periods=sma_period).mean(),
        )
    ...
    for i in range(lookback + 20, len(data_15m)):
        ts = cast(pd.Timestamp, data_15m.index[i])
        close = closes[i]
        high_val = highs[i]
        low_val = lows[i]
        ...
        if current_signal is not None and sma_period > 0:
            ...
            if s1h is not None and s30m is not None and s15m is not None:
                ...
                c1h = cast(float | None, close_1h_series.asof(ts))
                c30m = cast(float | None, close_30m_series.asof(ts))
                close_1h = None if c1h is None or pd.isna(c1h) else float(c1h)
                close_30m = None if c30m is None or pd.isna(c30m) else float(c30m)
...
def main() -> int:
    ...
    parser.add_argument(
        "--sma-period", type=int, default=50, help="SMA period for alignment gate (0=disabled)"
    )
    parser.add_argument(
